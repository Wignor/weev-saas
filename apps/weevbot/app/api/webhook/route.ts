...
function isAudioMessage(message: Record<string, unknown> | undefined): boolean {
      ...
      if (classification) {
        ...
        if (classification === 'cliente') {
          ...
          if ((await redis.get(KEYS.atendimento(number))) !== 'humano') {
            ...
            rescheduleLeadFollowups(number).catch(() => {});
          }
        }
      } else {
...
async function rescheduleLeadFollowups(number: string): Promise<void> {
  try {
    const configs = await getFollowupConfigs();
    if (configs.some(c => c.enabled && c.message?.trim())) {
      await scheduleFollowups(number, configs);
    }
  } catch {}
}
...
export async function POST(req: Request) {
  try {
    ...
    if (await redis.get(KEYS.botBlock(number))) return NextResponse.json({ ok: true });
    ...
    if (!text.trim()) {
      ...
      const base64 = await getMediaBase64(data.key, data.message);
      if (!base64) {
    ...
    // Cancel pending follow-ups since the lead responded
    if (contact?.status === 'lead') cancelFollowups(number).catch(() => {});
