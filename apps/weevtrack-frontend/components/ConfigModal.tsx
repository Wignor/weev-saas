...
export default function ConfigModal({ onClose }: Props) {
  ...
  const [tplForm, setTplForm] = useState({ name: '', description: '', installationValue: '', monthlyValue: '' });
  ...
  async function addSignatory() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        flash(`❌ ${data.error || 'Erro ao adicionar'}`);
      }
  ...
  async function deleteSignatory(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        flash(`❌ ${data.error || 'Erro ao remover'}`);
      }
    } catch { flash('❌ Erro de conexão'); }
    ...
    try {
      const res = await fetch('/api/templates', {
        ...
        body: JSON.stringify({
          ...
          installationValue: parseFloat(tplForm.installationValue.replace(',', '.')) || 0,
          monthlyValue: parseFloat(tplForm.monthlyValue.replace(',', '.')) || 0,
        }),
      });
      const data = await res.json();
      if (res.ok) {
        setTemplates(prev => [...prev, data]);
        setTplForm({ name: '', description: '', installationValue: '', monthlyValue: '' });
  ...
  const labelStyle = { margin: '0 0 5px', fontSize: 12, fontWeight: 600 as const, color: 'var(--text-lo)' };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 3000, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div style={{ width: '100%', maxHeight: '92dvh', background: 'var(--bg-card)', borderRadius: '20px 20px 0 0', display: 'flex', flexDirection: 'column' }}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 32px' }}>
          ...
          {tab === 'signatarios' && (
            <>
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-lo)', marginBottom: 10 }}>
                Signatários cadastrados
              </p>

              {loadingSig ? (
                <div style={{ textAlign: 'center', padding: 20 }}>
                  <div style={{ width: 22, height: 22, border: '2px solid #007AFF', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto' }} />
                </div>
              ) : (
                    ...
                    <div key={s.id} style={{ padding: '12px 14px', borderRadius: 12, background: 'var(--bg-input)', border: '1px solid var(--bg-border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                          ...
                          {s.isDefault && (
                            <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 4, background: 'rgba(0,122,255,0.15)', color: '#007AFF', fontWeight: 700 }}>padrão</span>
                          )}
                        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <button onClick={addSignatory} disabled={savingSig || !sigForm.name || !sigForm.cpf}
                  style={{ width: '100%', padding: '13px', borderRadius: 12, border: 'none', cursor: sigForm.name && sigForm.cpf ? 'pointer' : 'not-allowed',
          ...
          {tab === 'modelos' && (
            <>
              ...
              {loadingTpl ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                  {templates.map(t => (
                    <div key={t.id} style={{ padding: '12px 14px', borderRadius: 12, background: 'var(--bg-input)', border: '1px solid var(--bg-border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                      ...
                      {t.isCustom && (
                        <button onClick={() => deleteTemplate(t.id)} disabled={deletingTplId === t.id}
                          style={{ fontSize: 11, padding: '4px 10px', borderRadius: 8, background: 'rgba(255,59,48,0.1)', color: '#FF3B30', border: 'none', cursor: 'pointer', fontWeight: 600, opacity: deletingTplId === t.id ? 0.5 : 1, flexShrink: 0 }}>
                          🗑 Remover
                        </button>
                      )}
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <p style={labelStyle}>Nome do modelo *</p>
                  <input value={tplForm.name} onChange={e => setTplForm(prev => ({ ...prev, name: e.target.value }))}
                    placeholder="Ex: Rastreamento 120/45" style={inputStyle} />
                </div>
                <div>
                  <p style={labelStyle}>Descrição (aparece no seletor)</p>
                  <input value={tplForm.description} onChange={e => setTplForm(prev => ({ ...prev, description: e.target.value }))}
                    placeholder="Ex: Instalação R$120 + Mensalidade R$45" style={inputStyle} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div>
                    <p style={labelStyle}>Valor instalação (R$)</p>
                    <input value={tplForm.installationValue} onChange={e => setTplForm(prev => ({ ...prev, installationValue: e.target.value }))}
                      placeholder="120,00" inputMode="decimal" style={inputStyle} />
                  </div>
                  <div>
                    <p style={labelStyle}>Mensalidade (R$)</p>
                    <input value={tplForm.monthlyValue} onChange={e => setTplForm(prev => ({ ...prev, monthlyValue: e.target.value }))}
                      placeholder="45,00" inputMode="decimal" style={inputStyle} />
                  </div>
                </div>
                <button onClick={addTemplate} disabled={savingTpl || !tplForm.name}
                  style={{ width: '100%', padding: '13px', borderRadius: 12, border: 'none', cursor: tplForm.name ? 'pointer' : 'not-allowed',
                    background: tplForm.name ? '#007AFF' : 'var(--bg-border)',
                    color: tplForm.name ? 'white' : 'var(--text-lo)', fontWeight: 700, fontSize: 14, opacity: savingTpl ? 0.6 : 1 }}>
