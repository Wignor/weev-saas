  ...
  for (const f of followups) {
    try {
      ...
      if (f.followup_message?.trim()) {
        await sendMessage(f.evolution_instance, remoteJid, f.followup_message, 1000);
      }

      if (f.file_url) {
        if (f.file_type === 'video') {
          await sendVideo(f.evolution_instance, remoteJid, f.file_url, '').catch(() => {});
        } else if (f.file_type === 'document') {
          await sendDocument(f.evolution_instance, remoteJid, f.file_url, f.file_name || 'documento.pdf').catch(() => {});
        }
      }
